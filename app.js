...
function part(i,k){const b=document.createElement('button');b.type='button';b.className='part '+(fs.selected.i===i&&fs.selected.k===k?'selected':'');b.textContent=fs.terms[i][k]||'0';b.onclick=e=>{e.preventDefault();fs.selected={i,k};drawF()};return b}
function opbtn(i){const b=document.createElement('button');b.type='button';b.className='fraction-operator select-op';b.textContent=fs.terms[i].op||'+';b.onclick=e=>{e.preventDefault();const ops=['+','−','×','÷'];const z=ops.indexOf(fs.terms[i].op);fs.terms[i].op=ops[(z+1)%4];drawF()};return b}
...
fraction.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.fv!==undefined){'+−×÷'.includes(b.dataset.fv)?fop(b.dataset.fv):fd(b.dataset.fv);return}switch(b.dataset.fa){case'clear':fs={mixed:false,terms:[{w:'',n:'',d:'',op:null}],selected:{i:0,k:'n'}};drawF();break;case'delete':fdel();break;case'add':fadd();break;case'deleteFrac':fremove();break;case'mixed':fs.mixed=!fs.mixed;fs.selected.k=fs.mixed?'w':'n';drawF();break;case'equals':feq();break}});
